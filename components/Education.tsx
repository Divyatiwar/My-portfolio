...
export default function Education() {
  ...
  return (
    <section id="education" ref={ref} className="py-20 bg-gray-900/30 section-padding">
      <motion.div
        initial={{ opacity: 0, y: 50 }}
        animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 50 }}
        transition={{ duration: 0.5 }}
      >
        <h2 className="text-3xl md:text-4xl font-bold mb-12 gradient-text text-center">Education</h2>
            ...
            >
              ...
              <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-gray-900/50 p-6 rounded-lg border border-gray-800 shadow-lg hover:border-purple-500 transition-all">
                <div className="flex items-center justify-between space-x-2 mb-1">
                  ...
                  <time className="text-sm text-purple-400 font-semibold">{item.year}</time>
                </div>
                <div className="text-gray-300">{item.institution}</div>
                <div className="text-purple-400 font-medium mt-1">{item.grade}</div>
              </div>
            </motion.div>
          ))}
        </div>
